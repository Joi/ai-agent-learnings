...
- **Meeting transcripts.** [Granola](https://www.granola.so/) runs during every meeting, producing timestamped transcripts with speaker identification. [muesli](https://github.com/harperreed/muesli) (a Rust CLI by Harper Reed) syncs those transcripts into a local DuckDB database with metadata (attendees, labels) and a vector index for semantic search. A post-processing step translates non-English transcripts, generates summaries, and injects them into Obsidian daily notes. A separate extractor then scans those summaries and creates intake files for notable concepts, people, and organizations mentioned in meetings.
...
**The graph is queryable without a database.** Obsidian renders the link graph visually. [QMD](https://qmd.io/) indexes it for semantic search. An AI agent can traverse it by reading files and following wikilinks. The "database" is the filesystem itself -- files are nodes, wikilinks are edges. This is not a metaphor; it's literally how graph traversal works in the system.
...
Luhmann's original rules map directly to the system:

| Luhmann's Rule | In This System |
|----------------|----------------|
| One idea per card | One concept per `.md` file |
| Permanent unique identifier | Filename (kebab-case, never renamed after promotion) |
| Links between cards | `[[wikilinks]]` in the body text |
| Index cards for entry points | `INDEX.md` (auto-generated by maintainer) |
| Bibliography cards | `type: reference` files with `source_url:` |
| Don't delete, only add | Intake is append-only; atlas files are amended, not replaced |

The one addition Luhmann didn't need: the `description:` field. He could glance at a handwritten card and know what it said. An AI agent scanning 4,300 files needs a machine-readable summary in the frontmatter. This single convention -- every file must have a `description:` that differs from its title -- is what makes the Zettelkasten approach work at scale with agents.

Sönke Ahrens' [*How to Take Smart Notes*](https://www.soenkeahrens.de/en/takesmartnotes) (2017) is the best modern treatment of why this works. His key argument: the Zettelkasten externalizes not just information but the *structure* of your thinking. When an AI agent adds a wikilink between two notes, it's doing the same thing -- making a structural connection that didn't exist before, which any future reader (human or agent) can traverse.
...
| **Granola** | AI meeting notepad -- transcribes meetings with speaker identification | [granola.so](https://www.granola.so/) |
| **muesli** | Rust CLI that syncs Granola transcripts into DuckDB + vector index for semantic search | [github.com/harperreed/muesli](https://github.com/harperreed/muesli) |
...
- Luhmann, Niklas. "Communicating with Slip Boxes," 1981. [luhmann.surge.sh](https://luhmann.surge.sh/communicating-with-slip-boxes)
- Ahrens, Sönke. *How to Take Smart Notes*, 2017. [soenkeahrens.de](https://www.soenkeahrens.de/en/takesmartnotes)
- Schmidt, Johannes F.K. "Niklas Luhmann's Card Index," 2018. [sociologica.unibo.it](https://sociologica.unibo.it/article/view/8350)

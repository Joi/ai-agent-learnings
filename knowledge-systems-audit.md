...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  WhatsApp/Signal ──► jibot (jibotmac) ──► bookmark-relay ──┐           │
...
│ git sync → jibotmac (5 min)   │  │ (hybrid + LLM rerank)       │  │
└───────────────────────────────┘  └─────────────────────────────┘  │
                                                                     │
┌─────────────────────────────────────────────────────────────────────┘
│
...
### jibotmac Agents (via jibot-docs)
...
Joi's Laptop (source of truth)
  ~/switchboard ──► Syncthing ──► jibotmac ~/switchboard (subset)
                                             macazbd ~/switchboard (subset)
  ~/jibot-docs  ──► GitHub    ──► jibotmac ~/jibot-docs (every 5 min via launchd)
...
| OpenClaw | jibotmac :18789 | Node.js | Active (being replaced) |
| signal-cli | jibotmac :8080 | Java | Active, standalone |
| jimindbot (Ethoswarm) | cloud (Mind ID: 9730BD03) | Ethoswarm platform | Provisioned, testing |
| bookmark-relay | jibotmac :9999 | Node.js | Active |
| Syncthing | all machines | Go | Active |
| jibotmac-heartbeat | ~/scripts/ | bash (launchd) | Active |

### Planned / Not Yet Deployed

| Component | Location | Status |
|-----------|----------|--------|
| NanoClaw | github.com/qwibitai/nanoclaw | **DEPLOYED** on jibotmac, jibrain hook live |
...
| Amplifier bridge (GTD data → jibotmac) | Not started | Design exists in upgrade docs |
...
| Amplifier GTD | jibotmac agents | NO integration (key gap) | Gap |
...
| QMD | jibotmac agents | No access from jibotmac | Gap |
...
### Gap 2: No Amplifier → jibotmac bridge
GTD data (Apple Reminders, beads issues, daily focus) lives on Joi's laptop. jibotmac agents can't access it. The proactive agent (morning briefing, nudges) requires this bridge. Three designs exist (daily snapshot, REST API, hybrid) but none are built.
...
The "chanoyu pattern" for exporting concepts/, organizations/, and people-public/ to jibot-docs is designed but not built. jibotmac agents only see chanoyu/.

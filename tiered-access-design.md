...
> **Note**: This document is actively being refined. Joi is remote from jibotmac;
...
All of this runs on jibotmac (Mac Mini M4 Pro, 64GB), with jibot-3's community bot features (Slack, Discord, MUD) running alongside OpenClaw as a separate, focused process.

---

## Jibotmac Dedicated Accounts

**Key insight**: jibotmac has its own dedicated identities, separate from Joi's personal accounts. This significantly reduces risk and enables a more permissive security model.

| Service | Account | Purpose |
|---------|---------|---------|
| **Phone** | Dedicated SIM | WhatsApp, Signal, Telegram for jibot |
| **Apple ID** | jibot@ito.com | iCloud, Apple Reminders, iMessage |
| **Google** | jibot@ito.com | Gmail, Calendar, Drive |

### Security Implications

Because these are **dedicated jibot accounts** (not Joi's personal accounts):

1. **Email exposure is bounded** — If jibot@ito.com Gmail is compromised, Joi's personal email (joi@ito.com) is unaffected
2. **Calendar is jibot's view** — Events are copied/shared to jibot's calendar, not direct access to Joi's primary calendar
3. **Reminders are local to jibotmac** — Apple Reminders on jibotmac, synced via jibot@ito.com iCloud
4. **No OAuth to Joi's accounts** — The agent never holds tokens for joi@ito.com directly
...
```bash
gog gmail list -a jibot@ito.com "in:inbox is:unread" --max 20
gog gmail search -a jibot@ito.com "newer_than:7d" --max 10
...
- "What's in my inbox?" → jibot reads jibot@ito.com Gmail
- "Reply to the MIT email saying I'll attend" → jibot drafts and sends from jibot@ito.com
- "Add this to my calendar" → jibot creates event on jibot@ito.com calendar

**For Assistants:**
- "Check if there's anything urgent in the inbox" → Can read jibot@ito.com
- "Draft a reply to the venue about the event date" → Creates draft, shows for approval
- "What meetings does Joi have this week?" → Reads jibot@ito.com calendar
...
```json
{
  ...
  "bindings": [
    { "agentId": "owner",     "match": { "channel": "signal",   "peer": { "kind": "direct", "id": "+819048411965" }}},
    { "agentId": "owner",     "match": { "channel": "whatsapp", "peer": { "kind": "direct", "id": "+819048411965" }}},
    { "agentId": "owner",     "match": { "channel": "telegram", "peer": { "kind": "direct", "id": "JOIS_TELEGRAM_ID" }}},
    { "agentId": "owner",     "match": { "channel": "imessage", "peer": { "kind": "direct", "id": "joi@ito.com" }}},
...
- Amplifier bridge (unnecessary on jibotmac)
...
> The full ~/switchboard directory should NEVER be shared with jibotmac directly.

### Three Document Tiers

| Tier | Location | Access | Examples |
|------|----------|--------|---------|
| **Private** | ~/dotfiles-private (encrypted) | Joi only, age-encrypted | API keys, financial data, security configs |
| **Internal** | ~/switchboard (Joi's laptop only) | Owner agent only | Daily notes, private people notes, health data |
| **Shareable** | ~/jibot-docs/ (synced to jibotmac) | Owner + Assistant + Trusted | Chanoyu, public concepts, org profiles |

### Airgap Strategy (jibotmini)

The airgapped machine (jibotmini) can serve as a document staging area:

```
jibotmac (full access)
    │
    │  Automated sync of "ok if hacked" documents
    │  via cron + rsync over local network
    ▼
jibotmini (airgapped from internet)
    │
...
- Configure jibotmini access controls
...
3. **Phase 1 execution** — Move jibot-3 to jibotmac (low risk, immediate)
